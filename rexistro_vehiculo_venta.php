...
if (!empty($modelo) && !empty($cantidade) && !empty($descricion) && !empty($marca) && !empty($prezo) && !empty($foto) ) {
    ...
    if ($num_filas > 0){
        ...
        $update="UPDATE `vehiculo_venda` SET cantidade=cantidade +$cantidade where modelo='$modelo'";
        ...
        $insert = "INSERT INTO vehiculo_venda (modelo, cantidade, descricion, marca, prezo, foto) VALUES ('$modelo','$cantidade','$descricion','$marca','$prezo','$foto')";
